 ...
 * @group legacy
 */
class LingotekNodeLayoutBuilderSymmetricTranslationTest extends LingotekTestBase {
  ...
  public function testNodeWithLayoutBuilderSTBlockTranslation() {
    ...
    \Drupal::state()->set('lingotek.uploaded_content_type', 'node+layoutbuilderst');
    ...
    $this->assertCount(4, $data['layout_builder__translation']['components']);
    $this->assertEquals($data['layout_builder__translation']['components'][$uuidArticlePoweredBy]['label'], 'Block title layout builder override');
    $this->assertEquals($data['layout_builder__translation']['components'][$uuidArticleRichText]['label'], 'Rich text label');
    $this->assertEquals($data['layout_builder__translation']['components'][$uuidArticleRichText]['rich_text.value'], 'Rich text value');
    // We don't include non translatable properties.
    $this->assertFalse(isset($data['layout_builder__translation']['components'][$uuidArticleRichText]['rich_text.format']));
  ...
  public function testNodeWithLayoutBuilderSTCustomBlockTranslation() {
    // This is a hack for avoiding writing different lingotek endpoint mocks.
    \Drupal::state()->set('lingotek.uploaded_content_type', 'node+layoutbuilderstcustom');
    ...
    $this->assertEquals($data['title'][0]['value'], 'Llamas are cool');
    $this->assertEquals($data['body'][0]['value'], 'Llamas are very cool');
    $this->assertCount(3, $data['layout_builder__translation']['components']);
    $this->assertEquals($data['layout_builder__translation']['components'][$uuidArticleContentBlock]['label'], 'Overridden block with Dogs title');
    $this->assertCount(1, $data['layout_builder__translation']['entities']);
    $this->assertCount(1, $data['layout_builder__translation']['entities']['block_content']);

    $this->assertUploadedDataFieldCount($data['layout_builder__translation']['entities']['block_content'][1], 1);
    $this->assertEquals($data['layout_builder__translation']['entities']['block_content'][1]['body'][0]['value'], 'Block Dogs are very cool');
    $this->assertEquals($data['layout_builder__translation']['entities']['block_content'][1]['_lingotek_metadata']['_entity_type_id'], 'block_content');
    $this->assertEquals($data['layout_builder__translation']['entities']['block_content'][1]['_lingotek_metadata']['_entity_id'], '1');
