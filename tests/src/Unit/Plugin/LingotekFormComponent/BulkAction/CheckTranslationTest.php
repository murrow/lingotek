...
use Drupal\lingotek\Plugin\LingotekFormComponent\BulkAction\CheckTranslation;
use Drupal\Tests\UnitTestCase;

/**
 * Unit test for the assign profile bulk action form component.
 *
 * @coversDefaultClass \Drupal\lingotek\Plugin\LingotekFormComponent\BulkAction\CheckTranslation
 * @group lingotek
 * @preserve GlobalState disabled
 */
class CheckTranslationTest extends UnitTestCase {
  ...
  protected function setupAction($langcode = 'it') {
    $this->action = new CheckTranslation([], 'check_translation', [
  ...
  public function testExecuteSingleWithNoTranslatableContent() {
    ...
    $this->messenger->expects($this->once())
      ->method('addWarning')
      ->with($this->translation->translate('Cannot check @type %label translation to @language. That @bundle_label is not enabled for translation.', [
        '@type' => 'My Bundle',
        '%label' => 'My entity',
  ...
  public function testExecuteSingleWithNoTranslatableConfiguredContent() {
    ...
    $this->messenger->expects($this->once())
      ->method('addWarning')
      ->with($this->translation->translate('Cannot check @type %label translation to @language. That @bundle_label is not enabled for Lingotek translation.', [
        '@type' => 'My Bundle',
        '%label' => 'My entity',
        '@bundle_label' => 'My bundle label',
        '@language' => 'it',
  ...
  public function testExecuteSingleWithApiException() {
    ...
    $this->messenger->expects($this->once())
      ->method('addError')
      ->with($this->translation->translate('The request for @entity_type %title translation status failed. Please try again.', [
