...
use Drupal\lingotek\Plugin\LingotekFormComponent\BulkAction\Cancel;
use Drupal\Tests\UnitTestCase;

/**
 * Unit test for the assign profile bulk action form component.
 *
 * @coversDefaultClass \Drupal\lingotek\Plugin\LingotekFormComponent\BulkAction\Cancel
 * @group lingotek
...
class CancelTest extends UnitTestCase {
   ...
   *
   * @var \Drupal\lingotek\Plugin\LingotekFormComponent\BulkAction\Cancel
   */
  protected $action;
  ...
  protected function setupAction($profile_id = 'custom_profile') {
    $this->action = new Cancel([], 'cancel_document', [
      'id' => 'cancel_document',
  ...
  public function testExecuteSingleWithNoTranslatableContent() {
    ...
    $this->messenger->expects($this->once())
      ->method('addWarning')
      ->with($this->translation->translate('Cannot cancel @type %label. That @bundle_label is not enabled for translation.', [
        '@type' => 'My Bundle',
        '%label' => 'My entity',
  ...
  public function testExecuteSingleWithNoTranslatableConfiguredContent() {
    ...
    $this->messenger->expects($this->once())
      ->method('addWarning')
      ->with($this->translation->translate('Cannot cancel @type %label. That @bundle_label is not enabled for Lingotek translation.', [
        '@type' => 'My Bundle',
        '%label' => 'My entity',
        '@bundle_label' => 'My bundle label',
      ]));
  ...
  public function testExecuteSingleWithApiException() {
    ...
    $this->messenger->expects($this->once())
      ->method('addError')
      ->with($this->translation->translate('The cancellation of @entity_type %title failed. Please try again.', [
