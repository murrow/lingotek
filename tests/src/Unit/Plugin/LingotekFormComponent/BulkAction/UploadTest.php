...
use Drupal\lingotek\Plugin\LingotekFormComponent\BulkAction\Upload;
use Drupal\Tests\UnitTestCase;

/**
 * Unit test for the assign profile bulk action form component.
 *
 * @coversDefaultClass \Drupal\lingotek\Plugin\LingotekFormComponent\BulkAction\Upload
 * @group lingotek
...
class UploadTest extends UnitTestCase {
   ...
   *
   * @var \Drupal\lingotek\Plugin\LingotekFormComponent\BulkAction\Upload
   */
  protected $action;
  ...
  public function testExecuteSingleWithNoTranslatableContent() {
    ...
    $this->messenger->expects($this->once())
      ->method('addWarning')
      ->with($this->translation->translate('Cannot upload @type %label. That @bundle_label is not enabled for translation.', [
        '@type' => 'My Bundle',
        '%label' => 'My entity',
  ...
  public function testExecuteSingleWithNoTranslatableConfiguredContent() {
    ...
    $this->messenger->expects($this->once())
      ->method('addWarning')
      ->with($this->translation->translate('Cannot upload @type %label. That @bundle_label is not enabled for Lingotek translation.', [
        '@type' => 'My Bundle',
        '%label' => 'My entity',
        '@bundle_label' => 'My bundle label',
      ]));
  ...
  public function testExecuteSingle() {
    ...
    $result = $this->action->executeSingle($entity, ['job_id' => 'my_job_id'], $executor, $context);
  ...
  public function testExecuteSingleWithApiException() {
    ...
    $this->messenger->expects($this->once())
      ->method('addError')
      ->with($this->translation->translate('The upload for @entity_type %title failed. Please try again.', [
    ...
    $this->lingotekContentTranslation->expects($this->exactly(2))
      ...
      ->willReturnCallback(
        function () {
          ...
          switch ($counter++) {
            case 0:
              throw new LingotekDocumentArchivedException('error calling api');
